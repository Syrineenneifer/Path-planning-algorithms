...
class AStarPlanner(object):
    ...
    def plan(self, start_state, dest_state):
        ...
        Q = priority_dict()
        Q[start_state] = sqrt((dest_state.x - start_state.x)**2 + (dest_state.y - start_state.y)**2) #I'm using the Euclidean distance

        # Array that contains the optimal distance we've found from the starting state so far
        best_dist_found = float("inf") * np.ones((world.shape[1], world.shape[0])) #g
        ...
        while Q:
            ...
            for ns in self.get_neighboring_states(s):
                ...
                heuristic_func = sqrt((dest_state.x - ns.x)**2 + (dest_state.y - ns.y)**2)
                ...
                if (ns not in Q) or (alternative_best_dist_ns < best_dist_found[ns.x, ns.y]):
                    Q[ns] = alternative_best_dist_ns + heuristic_func
